...
export default function QuizClient({ quiz, lesson }) {
  ...
  async function finishQuiz() {
    ...
    quiz.questions.forEach(q => {
      ...
      if (q.type === 'mcq') {
        ...
        if (typeof userAnswer === 'string' && userAnswer.toLowerCase() === q.correct.toLowerCase()) correctCount++;
      }
    });
  ...
  const progressPct = Math.round(((current + (submitted ? 1 : 0)) / totalQuestions) * 100);
    ...
    <main style={{ background: '#0a0e1a', minHeight: '100vh', color: '#e8eaf0', fontFamily: "'DM Sans',sans-serif", paddingTop: '80px' }}>
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '2rem 1.5rem 4rem' }}>
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          <Link href={`/lessons/${lesson.id}`} style={{ fontSize: '13px', color: '#3b82f6', textDecoration: 'none' }}>
        ...
        {!submitted ? (
          <>
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <p style={{ fontSize: '13px', color: '#8892a4' }}>Question {current + 1} of {totalQuestions}</p>
            </div>
            ...
            <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '2rem' }}>
              ...
              <h2 style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: '1.2rem', lineHeight: 1.4, marginBottom: '1.75rem' }}>{question.question}</h2>
                  ...
                  {question.options.map((opt, i) => (
                    <button key={i} onClick={() => selectMcq(i)} style={{
                      ...
                      <span style={{
                        ...
                        border: `1.5px solid ${answers[question.id] === i ? '#2563eb' : 'rgba(255,255,255,0.15)'}`,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                      }}>
                        {answers[question.id] === i && <span style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#2563eb' }} />}
                      </span>
                      {opt}
                    </button>
                  ))}
                ...
                <div style={{ display: 'flex', gap: '.75rem' }}>
                  {[true, false].map(val => (
                    <button key={String(val)} onClick={() => selectTrueFalse(val)} style={{
                      ...
                      color: answers[question.id] === val ? '#fff' : '#e8eaf0', fontSize: '14px', fontWeight: 500,
              ...
              {question.type === 'fillblank' && (
                <input
                  ...
                  style={{
                    ...
                    borderRadius: '10px', padding: '.85rem 1.1rem', color: '#e8eaf0', fontSize: '14px',
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '1.5rem' }}>
              <button onClick={goBack} disabled={current === 0} style={{
                ...
                background: 'transparent', color: current === 0 ? '#4a5568' : '#8892a4', fontSize: '13px',
                fontWeight: 500, cursor: current === 0 ? 'not-allowed' : 'pointer', fontFamily: "'DM Sans',sans-serif",
              }}>← Back</button>
              <button onClick={goNext} disabled={!isAnswered} style={{
                padding: '.7rem 1.75rem', borderRadius: '8px', border: 'none',
                background: isAnswered ? '#2563eb' : 'rgba(255,255,255,0.06)',
                color: isAnswered ? '#fff' : '#4a5568', fontSize: '13px', fontWeight: 600,
                cursor: isAnswered ? 'pointer' : 'not-allowed', fontFamily: "'DM Sans',sans-serif",
          ...
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '2.5rem', textAlign: 'center' }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '12px', padding: '1.25rem', marginBottom: '1.75rem' }}>
              <div style={{ height: '8px', background: 'rgba(255,255,255,0.08)', borderRadius: '4px', overflow: 'hidden', marginBottom: '.75rem' }}>
                <div style={{
                  height: '100%', width: `${result.scorePct}%`, borderRadius: '4px',
                  background: result.passed ? '#4ade80' : '#f59e0b', transition: 'width 0.5s',
                }} />
              </div>
            ...
            {result.passed ? (
              <div style={{ background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: '10px', padding: '1rem', marginBottom: '1.5rem', fontSize: '13px', color: '#4ade80' }}>
                ✓ Next lesson unlocked! You can now continue your BIM journey.
              </div>
            ) : (
              <div style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '10px', padding: '1rem', marginBottom: '1.5rem', fontSize: '13px', color: '#f59e0b' }}>
                You need {quiz.passingScore}% to pass and unlock the next lesson. Review the lesson and try again!
              </div>
            )}

            <div style={{ display: 'flex', gap: '.75rem', justifyContent: 'center', flexWrap: 'wrap' }}>
              <button onClick={retakeQuiz} style={{
                padding: '.75rem 1.5rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.08)',
                background: 'transparent', color: '#e8eaf0', fontSize: '13px', fontWeight: 500,
                cursor: 'pointer', fontFamily: "'DM Sans',sans-serif",
              }}>Retake Quiz</button>

              {result.passed && lesson.next ? (
                <Link href={`/lessons/${lesson.next.id}`} style={{
                  padding: '.75rem 1.5rem', borderRadius: '8px', background: '#2563eb', color: '#fff',
