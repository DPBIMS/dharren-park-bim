...
const tagStyle = {
  ...
  standards:    { bg:'rgba(6,182,212,0.12)',  color:'#22d3ee', border:'rgba(6,182,212,0.2)'  },
  coordination: { bg:'rgba(20,184,166,0.12)', color:'#2dd4bf', border:'rgba(20,184,166,0.2)' },
  navisworks:   { bg:'rgba(16,185,129,0.12)', color:'#34d399', border:'rgba(16,185,129,0.2)' },
  bim360:       { bg:'rgba(139,92,246,0.12)', color:'#a78bfa', border:'rgba(139,92,246,0.2)' },
  ifc:          { bg:'rgba(245,158,11,0.12)', color:'#fbbf24', border:'rgba(245,158,11,0.2)' },
};
...
const topicLabel = {
  general: 'General', revit: 'Revit', standards: 'Standards',
  coordination: 'Coordination', navisworks: 'Navisworks', bim360: 'BIM 360', ifc: 'IFC',
};
...
export default function BimBasicsPage() {
  ...
  const quizQuestions   = lessons.reduce((s, l) => s + (l.quiz?.questions || 10), 0);
    ...
    <main style={{ background:'#0a0e1a', color:'#e8eaf0', minHeight:'100vh', paddingTop:'80px', fontFamily:"'DM Sans',sans-serif" }}>
      <div style={{ maxWidth:'1100px', margin:'0 auto', padding:'3rem 2rem' }}>
        ...
        <div style={{ fontSize:'12px', color:'#8892a4', marginBottom:'1.5rem', display:'flex', alignItems:'center', gap:'.5rem' }}>
          ...
          <span>Learn</span>
          <span>›</span>
          <span style={{ color:'#e8eaf0' }}>BIM Basics</span>
        </div>

        {/* Header */}
        <div style={{ marginBottom:'2.5rem' }}>
          <div style={{ fontSize:'11px', fontWeight:500, letterSpacing:'2px', textTransform:'uppercase', color:'#ec4899', marginBottom:'.5rem' }}>Learn · Foundation</div>
          <h1 style={{ fontFamily:"'Syne',sans-serif", fontSize:'2.2rem', fontWeight:800, letterSpacing:'-.5px', marginBottom:'.5rem' }}>BIM Basics</h1>
          <p style={{ color:'#8892a4', fontSize:'15px' }}>Start here — the core concepts every BIM professional needs before diving into software, standards, and workflows.</p>
        </div>

        {/* Course stats strip */}
        <div style={{ display:'flex', flexWrap:'wrap', gap:'1.5rem', marginBottom:'2rem', padding:'1rem 1.5rem', background:'rgba(236,72,153,0.04)', border:'1px solid rgba(236,72,153,0.12)', borderRadius:'12px', fontSize:'13px' }}>
          {[
        ...
        {!loading && activePlan !== 'free' && (
          <div style={{ background:'rgba(236,72,153,0.06)', border:'1px solid rgba(236,72,153,0.2)', borderRadius:'10px', padding:'.75rem 1.25rem', marginBottom:'1.5rem', fontSize:'13px', color:'#f9a8d4', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            <span>✓ Logged in as <strong style={{ textTransform:'capitalize' }}>{activePlan} Plan</strong> — {unlockedCount} of {lessons.length} lessons unlocked · {completedCount} completed</span>
            <Link href="/dashboard" style={{ color:'#ec4899', fontSize:'12px', fontWeight:500, textDecoration:'none' }}>Go to Dashboard →</Link>
          </div>
        )}
        {!loading && activePlan === 'free' && (
          <div style={{ background:'rgba(245,158,11,0.06)', border:'1px solid rgba(245,158,11,0.2)', borderRadius:'10px', padding:'.75rem 1.25rem', marginBottom:'1.5rem', fontSize:'13px', color:'#fbbf24', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            <span>🔒 You have access to {freeCount} free lessons. Upgrade to unlock the full library.</span>
            ...
            {[['all','All Levels'],['beginner','Beginner'],['intermediate','Intermediate'],['advanced','Advanced']].map(([val,label]) => (
              <button key={val} onClick={() => setActiveCat(val)} style={{
                background: activeCat===val
                  ? val==='beginner'?'rgba(34,197,94,0.12)':val==='intermediate'?'rgba(245,158,11,0.12)':val==='advanced'?'rgba(239,68,68,0.12)':'rgba(236,72,153,0.15)'
                  : 'rgba(255,255,255,0.04)',
                border: `1px solid ${activeCat===val
                  ? val==='beginner'?'rgba(34,197,94,0.3)':val==='intermediate'?'rgba(245,158,11,0.3)':val==='advanced'?'rgba(239,68,68,0.3)':'rgba(236,72,153,0.4)'
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:'.4rem', marginBottom:'1.5rem' }}>
          {[['all','All Topics'], ...topics.map(t => [t, topicLabel[t] || t])].map(([val,label]) => (
                ...
                <div key={l.id} style={{ background:'rgba(255,255,255,0.04)', border:`1px solid ${isDone?'rgba(34,197,94,0.2)':accessible?'rgba(255,255,255,0.08)':'rgba(255,255,255,0.05)'}`, borderRadius:'14px', overflow:'hidden', display:'flex', flexDirection:'column', opacity:accessible?1:0.75 }}>
                  <div style={{ padding:'1.25rem 1.25rem .75rem', flex:1 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:'.4rem', marginBottom:'.75rem', flexWrap:'wrap' }}>
                      <span style={{ fontSize:'11px', fontWeight:500, padding:'3px 10px', borderRadius:'5px', background:ts.bg, color:ts.color, border:`1px solid ${ts.border}` }}>{topicLabel[l.topic] || l.topic}</span>
                  ...
                  <div style={{ padding:'.75rem 1.25rem', borderTop:'1px solid rgba(255,255,255,0.06)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                    ...
                    {accessible ? (
                      <Link href={`/lessons/${l.id}?from=bim-basics`} style={{ fontSize:'12px', color:'#ec4899', fontWeight:500, textDecoration:'none', display:'flex', alignItems:'center', gap:'4px' }}>
                        {isDone ? '✓ Review' : 'Read lesson'} <span>→</span>
                      </Link>
                    ) : (
                      <div style={{ display:'flex', alignItems:'center', gap:'.5rem' }}>
                        <span>🔒</span>
                        <Link href="/pricing" style={{ fontSize:'12px', color:'#ec4899', textDecoration:'none' }}>View Plans</Link>
                      </div>
                    )}
                  </div>
                </div>
