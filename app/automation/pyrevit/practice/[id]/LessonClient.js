...
export default function PyRevitPracticeLessonClient({ lesson, allLessons }) {
    ...
    <div style={{ fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ position:'fixed', top:'64px', left:0, right:0, zIndex:90, background:'rgba(10,14,26,0.95)', backdropFilter:'blur(12px)', borderBottom:'1px solid rgba(255,255,255,0.07)', height:'48px', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 1.5rem' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'1rem', fontSize:'13px' }}>
          <Link href="/automation/pyrevit/practice" style={{ color:'#8892a4', textDecoration:'none' }}>← Back to Practice Course</Link>
          ...
          <div style={{ maxWidth:'720px' }}>
            ...
            {lesson.quiz && (
              <div style={{ background:'rgba(37,99,235,0.06)', border:'1px solid rgba(37,99,235,0.2)', borderRadius:'14px', padding:'1.5rem', marginBottom:'2rem', display:'flex', alignItems:'center', justifyContent:'space-between', gap:'1rem', flexWrap:'wrap' }}>
                ...
                <Link href={`/automation/pyrevit/practice/${lesson.id}/quiz`} style={{ background:'#3b82f6', color:'#fff', padding:'10px 24px', borderRadius:'8px', fontSize:'13px', fontWeight:700, textDecoration:'none', whiteSpace:'nowrap' }}>
                  Take Quiz →
                </Link>
              </div>
            )}
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem', marginTop:'2rem' }}>
              {prevNav ? (
                <Link href={`/automation/pyrevit/practice/${prevNav.id}`} style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'12px', padding:'1rem 1.25rem', textDecoration:'none', display:'block' }}>
              ...
              {nextNav ? (
                nextIsLocked ? (
                  ...
                  <Link href={`/automation/pyrevit/practice/${nextNav.id}`} style={{ background:'rgba(37,99,235,0.06)', border:'1px solid rgba(37,99,235,0.2)', borderRadius:'12px', padding:'1rem 1.25rem', textDecoration:'none', display:'block' }}>
                ...
                <div style={{ background:'rgba(34,197,94,0.06)', border:'1px solid rgba(34,197,94,0.2)', borderRadius:'12px', padding:'1rem 1.25rem' }}>
                  ...
                  <div style={{ fontSize:'13px', fontWeight:600, color:'#e8eaf0' }}>You finished the pyRevit Hands-On Practice course!</div>
                </div>
              )}
            </div>
          </div>
          ...
          <div style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'12px', padding:'1.25rem' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'2px' }}>
              {allLessons.slice(0, 8).map(l => (
                <Link key={l.id} href={`/automation/pyrevit/practice/${l.id}`} style={{ fontSize:'12px', padding:'5px 8px', borderRadius:'6px', textDecoration:'none', color:l.id===lesson.id?'#60a5fa':'#8892a4', background:l.id===lesson.id?'rgba(37,99,235,0.08)':'transparent', display:'flex', alignItems:'center', gap:'6px' }}>
              ...
              {allLessons.length > 8 && (
                <Link href="/automation/pyrevit/practice" style={{ fontSize:'12px', color:'#3b82f6', padding:'5px 8px', textDecoration:'none' }}>
