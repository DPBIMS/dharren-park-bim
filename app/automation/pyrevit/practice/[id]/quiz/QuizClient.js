...
export default function PyRevitPracticeQuizClient({ quiz, lesson }) {
    ...
    <main style={{ background: '#0a0e1a', minHeight: '100vh', color: '#e8eaf0', fontFamily: "'DM Sans',sans-serif", paddingTop: '80px' }}>
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '2rem 1.5rem 4rem' }}>
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          <Link href={`/automation/pyrevit/practice/${lesson.id}`} style={{ fontSize: '13px', color: '#3b82f6', textDecoration: 'none' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '2.5rem', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '.75rem', justifyContent: 'center', flexWrap: 'wrap' }}>
              ...
              {result.passed && lesson.next ? (
                <Link href={`/automation/pyrevit/practice/${typeof lesson.next === 'string' ? lesson.next : lesson.next.id}`} style={{ padding: '.75rem 1.5rem', borderRadius: '8px', background: '#2563eb', color: '#fff', fontSize: '13px', fontWeight: 600, textDecoration: 'none' }}>
                  Next Lesson →
                </Link>
              ) : (
                <Link href={`/automation/pyrevit/practice/${lesson.id}`} style={{ padding: '.75rem 1.5rem', borderRadius: '8px', background: '#2563eb', color: '#fff', fontSize: '13px', fontWeight: 600, textDecoration: 'none' }}>
                  Review Lesson
                </Link>
              )}
            </div>
