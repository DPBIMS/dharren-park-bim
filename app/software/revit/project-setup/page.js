...
const topicColor = {
  Introduction:    { bg:'rgba(37,99,235,0.12)',  color:'#60a5fa', border:'rgba(37,99,235,0.2)'  },
  Foundation:      { bg:'rgba(16,185,129,0.12)', color:'#34d399', border:'rgba(16,185,129,0.2)' },
  Coordinates:     { bg:'rgba(139,92,246,0.12)', color:'#a78bfa', border:'rgba(139,92,246,0.2)' },
  Datum:           { bg:'rgba(245,158,11,0.12)', color:'#fbbf24', border:'rgba(245,158,11,0.2)' },
  Worksharing:     { bg:'rgba(20,184,166,0.12)', color:'#2dd4bf', border:'rgba(20,184,166,0.2)' },
  Standards:       { bg:'rgba(99,102,241,0.12)', color:'#818cf8', border:'rgba(99,102,241,0.2)' },
  Collaboration:   { bg:'rgba(236,72,153,0.12)', color:'#f472b6', border:'rgba(236,72,153,0.2)' },
  'Visual Standards': { bg:'rgba(6,182,212,0.12)', color:'#22d3ee', border:'rgba(6,182,212,0.2)' },
  Annotation:      { bg:'rgba(168,85,247,0.12)', color:'#c084fc', border:'rgba(168,85,247,0.2)' },
  'View Standards':{ bg:'rgba(37,99,235,0.12)',  color:'#60a5fa', border:'rgba(37,99,235,0.2)'  },
  Organization:    { bg:'rgba(107,114,128,0.12)',color:'#9ca3af', border:'rgba(107,114,128,0.2)' },
  Content:         { bg:'rgba(34,197,94,0.12)',  color:'#4ade80', border:'rgba(34,197,94,0.2)'  },
  Sheets:          { bg:'rgba(245,158,11,0.12)', color:'#fbbf24', border:'rgba(245,158,11,0.2)' },
  'Data Standards':{ bg:'rgba(239,68,68,0.12)',  color:'#f87171', border:'rgba(239,68,68,0.2)'  },
  Handover:        { bg:'rgba(16,185,129,0.12)', color:'#34d399', border:'rgba(16,185,129,0.2)' },
  'Model Quality': { bg:'rgba(99,102,241,0.12)', color:'#818cf8', border:'rgba(99,102,241,0.2)' },
  'Template Management': { bg:'rgba(245,158,11,0.12)', color:'#f59e0b', border:'rgba(245,158,11,0.2)' },
...
export default function RevitProjectSetupPage() {
  ...
  const filtered = revitSetupLessons.filter(l => {
  ...
  const unlockedCount  = revitSetupLessons.filter(l => canAccess(l)).length;
  const completedCount = revitSetupLessons.filter(l => completed.has(l.id)).length;
    ...
    <main style={{ background:'#0a0e1a', color:'#e8eaf0', minHeight:'100vh', paddingTop:'80px', fontFamily:"'DM Sans',sans-serif" }}>
      <div style={{ maxWidth:'1100px', margin:'0 auto', padding:'3rem 2rem' }}>
        ...
        <div style={{ fontSize:'12px', color:'#8892a4', marginBottom:'1.5rem', display:'flex', alignItems:'center', gap:'.5rem' }}>
          ...
          <span>›</span>
          <span style={{ color:'#e8eaf0' }}>Project Setup</span>
        </div>

        {/* Header */}
        <div style={{ marginBottom:'2.5rem' }}>
          <div style={{ fontSize:'11px', fontWeight:500, letterSpacing:'2px', textTransform:'uppercase', color:'#10b981', marginBottom:'.5rem' }}>
            Software · Revit
          </div>
          ...
          <p style={{ color:'#8892a4', fontSize:'15px', maxWidth:'680px' }}>
            Build your own ISO 19650-aligned Revit project template from absolute zero — units, coordinates, levels, grids, materials, view templates, shared parameters, IFC export, and a production-ready template you can use on real projects immediately.
          </p>
        </div>

        {/* Course stats strip */}
        <div style={{ display:'flex', flexWrap:'wrap', gap:'1.5rem', marginBottom:'2rem', padding:'1rem 1.5rem', background:'rgba(16,185,129,0.04)', border:'1px solid rgba(16,185,129,0.12)', borderRadius:'12px', fontSize:'13px' }}>
          {[
            ...
            { label:'End Result',     value:'Production-Ready .rte Template' },
            { label:'Plan Required',  value:'Free → Premium' },
          ].map(s => (
            <div key={s.label}>
        ...
        {/* What you will build banner */}
        <div style={{ background:'rgba(16,185,129,0.06)', border:'1px solid rgba(16,185,129,0.2)', borderRadius:'12px', padding:'1.25rem 1.5rem', marginBottom:'1.5rem' }}>
          <div style={{ display:'flex', alignItems:'flex-start', gap:'12px' }}>
            <span style={{ fontSize:'20px', flexShrink:0 }}>🏗</span>
            <div>
              <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:'14px', color:'#34d399', marginBottom:'.3rem' }}>
                What you will build in this course
              </div>
              <div style={{ fontSize:'13px', color:'#8892a4', lineHeight:'1.7' }}>
                The <strong style={{ color:'#e8eaf0' }}>Dharren Park Commercial Centre</strong> Revit project file — a 6-storey mixed-use building configured from a blank template to a production-ready ISO 19650-compliant model. Every lesson adds one layer of professional configuration. By Lesson 24 you save it as an office template (.rte) you can use on real projects immediately.
              </div>
            </div>
          </div>
        </div>
        ...
        {!loading && activePlan !== 'free' && (
          <div style={{ background:'rgba(37,99,235,0.06)', border:'1px solid rgba(37,99,235,0.2)', borderRadius:'10px', padding:'.75rem 1.25rem', marginBottom:'1.5rem', fontSize:'13px', color:'#93c5fd', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            <span>✓ Logged in as <strong style={{ textTransform:'capitalize' }}>{activePlan} Plan</strong> — {unlockedCount} of {revitSetupLessons.length} lessons unlocked · {completedCount} completed</span>
            <Link href="/dashboard" style={{ color:'#3b82f6', fontSize:'12px', fontWeight:500, textDecoration:'none' }}>Go to Dashboard →</Link>
          </div>
        )}
        {!loading && activePlan === 'free' && (
          <div style={{ background:'rgba(16,185,129,0.06)', border:'1px solid rgba(16,185,129,0.2)', borderRadius:'10px', padding:'.75rem 1.25rem', marginBottom:'1.5rem', fontSize:'13px', color:'#34d399', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            <span>✓ {freeCount} free lessons available. Upgrade to unlock the full hands-on build.</span>
            <Link href="/pricing" style={{ color:'#10b981', fontSize:'12px', fontWeight:600, textDecoration:'none' }}>Upgrade Now →</Link>
          </div>
        )}
            ...
            {[['all','All Levels'],['beginner','Beginner'],['intermediate','Intermediate'],['advanced','Advanced']].map(([val, label]) => (
              <button key={val} onClick={() => setActiveCat(val)} style={{
                background: activeCat === val
                  ? val==='beginner' ? 'rgba(34,197,94,0.12)' : val==='intermediate' ? 'rgba(245,158,11,0.12)' : val==='advanced' ? 'rgba(239,68,68,0.12)' : 'rgba(16,185,129,0.15)'
                  : 'rgba(255,255,255,0.04)',
                border: `1px solid ${activeCat === val
                  ? val==='beginner' ? 'rgba(34,197,94,0.3)' : val==='intermediate' ? 'rgba(245,158,11,0.3)' : val==='advanced' ? 'rgba(239,68,68,0.3)' : 'rgba(16,185,129,0.4)'
            ...
            {filtered.map(l => {
              ...
              return (
                <div key={l.id} style={{
                  ...
                  border:`1px solid ${isDone ? 'rgba(16,185,129,0.25)' : accessible ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.05)'}`,
                  ...
                  <div style={{ padding:'1.25rem 1.25rem .75rem', flex:1 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:'.4rem', marginBottom:'.75rem', flexWrap:'wrap' }}>
                      ...
                      {isDone && (
                        <span style={{ fontSize:'10px', fontWeight:600, padding:'2px 8px', borderRadius:'4px', background:'rgba(16,185,129,0.1)', color:'#34d399' }}>
                          ✓ Done
                        </span>
                      )}
                    </div>
                  ...
                  <div style={{ padding:'.75rem 1.25rem', borderTop:'1px solid rgba(255,255,255,0.06)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                    ...
                    {accessible ? (
                      <Link href={`/software/revit/project-setup/${l.id}`} style={{ fontSize:'12px', color:'#10b981', fontWeight:500, textDecoration:'none', display:'flex', alignItems:'center', gap:'4px' }}>
                        {isDone ? '✓ Review' : 'Read lesson'} <span>→</span>
                      </Link>
                    ) : (
                      <div style={{ display:'flex', alignItems:'center', gap:'.5rem' }}>
                        <span>🔒</span>
                        <Link href="/pricing" style={{ fontSize:'12px', color:'#10b981', textDecoration:'none' }}>View Plans</Link>
                      </div>
                    )}
                  </div>
                </div>
