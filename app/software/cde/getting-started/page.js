...
const tagStyle = {
  'CDE Fundamentals':      { bg:'rgba(37,99,235,0.12)',  color:'#60a5fa', border:'rgba(37,99,235,0.2)'  },
  'CDE States':            { bg:'rgba(139,92,246,0.12)', color:'#a78bfa', border:'rgba(139,92,246,0.2)' },
  'Naming & Metadata':     { bg:'rgba(20,184,166,0.12)', color:'#2dd4bf', border:'rgba(20,184,166,0.2)' },
  'Roles & Governance':    { bg:'rgba(245,158,11,0.12)', color:'#fbbf24', border:'rgba(245,158,11,0.2)' },
  'Suitability Codes':     { bg:'rgba(234,179,8,0.12)',  color:'#facc15', border:'rgba(234,179,8,0.2)'  },
  'ACC Configuration':     { bg:'rgba(34,197,94,0.12)',  color:'#4ade80', border:'rgba(34,197,94,0.2)'  },
  'Model Federation':      { bg:'rgba(14,165,233,0.12)', color:'#38bdf8', border:'rgba(14,165,233,0.2)' },
  'Transmittals':          { bg:'rgba(236,72,153,0.12)', color:'#f472b6', border:'rgba(236,72,153,0.2)' },
  'Revisions':             { bg:'rgba(99,102,241,0.12)', color:'#818cf8', border:'rgba(99,102,241,0.2)' },
  'Drawing Workflow':      { bg:'rgba(239,68,68,0.12)',  color:'#f87171', border:'rgba(239,68,68,0.2)'  },
  'RFI & Issues':          { bg:'rgba(245,158,11,0.12)', color:'#f59e0b', border:'rgba(245,158,11,0.2)' },
  'Multi-Party CDE':       { bg:'rgba(168,85,247,0.12)', color:'#c084fc', border:'rgba(168,85,247,0.2)' },
  'BEP & Governance':      { bg:'rgba(16,185,129,0.12)', color:'#34d399', border:'rgba(16,185,129,0.2)' },
  'Clash Detection':       { bg:'rgba(239,68,68,0.12)',  color:'#fb7185', border:'rgba(239,68,68,0.2)'  },
  'Project Stages':        { bg:'rgba(59,130,246,0.12)', color:'#93c5fd', border:'rgba(59,130,246,0.2)' },
  'Submittals & Approvals':{ bg:'rgba(20,184,166,0.12)', color:'#5eead4', border:'rgba(20,184,166,0.2)' },
  'COBie & Data':          { bg:'rgba(34,197,94,0.12)',  color:'#86efac', border:'rgba(34,197,94,0.2)'  },
  'As-Built & Handover':   { bg:'rgba(16,185,129,0.12)', color:'#6ee7b7', border:'rgba(16,185,129,0.2)' },
  'CDE Governance':        { bg:'rgba(107,114,128,0.12)',color:'#d1d5db', border:'rgba(107,114,128,0.2)' },
  'CDE Security':          { bg:'rgba(239,68,68,0.12)',  color:'#ef4444', border:'rgba(239,68,68,0.2)'  },
  'Office Setup':          { bg:'rgba(245,158,11,0.12)', color:'#fcd34d', border:'rgba(245,158,11,0.2)' },
  'Onboarding & Training': { bg:'rgba(99,102,241,0.12)', color:'#a5b4fc', border:'rgba(99,102,241,0.2)' },
...
export default function CDEGettingStartedPage() {
  ...
  const filtered = cdeLessons.filter(l => {
  ...
  const topics         = [...new Set(cdeLessons.map(l => l.topic))];
  const unlockedCount  = cdeLessons.filter(l => canAccess(l)).length;
  const completedCount = cdeLessons.filter(l => completed.has(l.id)).length;
  const quizQuestions  = cdeLessons.reduce((s, l) => s + (l.quiz?.questions || 10), 0);
    ...
    <main style={{ background:'#0a0e1a', color:'#e8eaf0', minHeight:'100vh', paddingTop:'80px', fontFamily:"'DM Sans',sans-serif" }}>
      <div style={{ maxWidth:'1100px', margin:'0 auto', padding:'3rem 2rem' }}>
        ...
        <div style={{ fontSize:'12px', color:'#8892a4', marginBottom:'1.5rem', display:'flex', alignItems:'center', gap:'.5rem' }}>
          ...
          <span>›</span>
          <span style={{ color:'#e8eaf0' }}>Common Data Environment — Getting Started</span>
        </div>
        ...
        <div style={{ marginBottom:'2.5rem' }}>
          ...
          <h1 style={{ fontFamily:"'Syne',sans-serif", fontSize:'2.2rem', fontWeight:800, letterSpacing:'-.5px', marginBottom:'.5rem' }}>Getting Started with the Common Data Environment</h1>
          <p style={{ color:'#8892a4', fontSize:'15px' }}>From CDE foundations and the four ISO 19650 states through federation, coordination, governance, and a full capstone handover — a complete practitioner's guide to the CDE.</p>
        </div>
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:'1.5rem', marginBottom:'2rem', padding:'1rem 1.5rem', background:'rgba(37,99,235,0.04)', border:'1px solid rgba(37,99,235,0.12)', borderRadius:'12px', fontSize:'13px' }}>
          {[
            { label:'Total Lessons',  value:String(cdeLessons.length) },
        ...
        {!loading && activePlan !== 'free' && (
          <div style={{ background:'rgba(37,99,235,0.06)', border:'1px solid rgba(37,99,235,0.2)', borderRadius:'10px', padding:'.75rem 1.25rem', marginBottom:'1.5rem', fontSize:'13px', color:'#93c5fd', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            <span>✓ Logged in as <strong style={{ textTransform:'capitalize' }}>{activePlan} Plan</strong> — {unlockedCount} of {cdeLessons.length} lessons unlocked · {completedCount} completed</span>
            <Link href="/dashboard" style={{ color:'#3b82f6', fontSize:'12px', fontWeight:500, textDecoration:'none' }}>Go to Dashboard →</Link>
          </div>
        )}
        {!loading && activePlan === 'free' && (
          <div style={{ background:'rgba(37,99,235,0.06)', border:'1px solid rgba(37,99,235,0.2)', borderRadius:'10px', padding:'.75rem 1.25rem', marginBottom:'1.5rem', fontSize:'13px', color:'#93c5fd', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            <span>🔒 You have access to {cdeLessons.filter(l=>l.free).length} free lessons. Upgrade to unlock the full CDE course.</span>
        ...
        <div style={{ display:'flex', gap:'1.5rem', marginBottom:'2rem', flexWrap:'wrap', alignItems:'center', fontSize:'13px', color:'#8892a4' }}>
          ...
          <span><strong style={{ color:'#e8eaf0' }}>{cdeLessons.filter(l=>l.free).length}</strong> free previews</span>
          <span style={{ width:'4px', height:'4px', borderRadius:'50%', background:'rgba(255,255,255,0.08)', display:'inline-block' }}/>
              ...
              return (
                <div key={l.id} style={{ background:'rgba(255,255,255,0.04)', border:`1px solid ${isDone?'rgba(34,197,94,0.2)':accessible?'rgba(255,255,255,0.08)':'rgba(255,255,255,0.05)'}`, borderRadius:'14px', overflow:'hidden', display:'flex', flexDirection:'column', opacity:accessible?1:0.75 }}>
                  ...
                  <div style={{ padding:'.75rem 1.25rem', borderTop:'1px solid rgba(255,255,255,0.06)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                    ...
                    {accessible ? (
                      <Link href={`/software/cde/getting-started/${l.id}`} style={{ fontSize:'12px', color:'#3b82f6', fontWeight:500, textDecoration:'none', display:'flex', alignItems:'center', gap:'4px' }}>
