...
const plans = [
  { name: 'Basic', desc: 'For beginners starting their BIM journey.', monthly: '₱299', annual: '₱239', features: ['Full access to BIM articles','Core lessons & topics','Beginner learning path'], unavail: ['Downloadable PDFs','Quizzes & assessments','Video lessons'], featured: false },
  { name: 'Pro', desc: 'For students serious about BIM careers.', monthly: '₱599', annual: '₱479', features: ['Everything in Basic','Downloadable PDFs','Quizzes & assessments','Intermediate lessons'], unavail: ['Video lessons','3D model resources'], featured: true },
  { name: 'Premium', desc: 'Full access to everything, now and future.', monthly: '₱999', annual: '₱799', features: ['Everything in Pro','Video lessons (coming soon)','3D model resources','Priority support','Early access to new content','Certificate of completion'], unavail: [], featured: false },
];
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '1.25rem', marginBottom: '2.5rem' }}>
          {plans.map(p => (
            <div key={p.name} onClick={() => setSelectedPlan(p.name)} style={{ background: selectedPlan === p.name ? 'rgba(37,99,235,0.07)' : 'rgba(255,255,255,0.04)', border: `1px solid ${selectedPlan === p.name ? '#2563eb' : p.featured ? 'rgba(37,99,235,0.4)' : 'rgba(255,255,255,0.08)'}`, borderRadius: '16px', padding: '1.75rem', position: 'relative', cursor: 'pointer', transition: 'all .2s' }}>
              ...
              <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: '15px', marginBottom: '.2rem' }}>{p.name}</div>
              ...
              <div style={{ marginTop: '1.25rem', height: '2px', borderRadius: '2px', background: selectedPlan === p.name ? '#2563eb' : 'rgba(255,255,255,0.06)', transition: 'background .2s' }} />
            </div>
          ))}
        </div>
        ...
        {!submitted ? (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', alignItems: 'start' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '.5rem', marginBottom: '1.25rem' }}>
                {methods.map(m => (
                  <div key={m.id} onClick={() => setSelectedMethod(m.id)} style={{ display: 'flex', alignItems: 'center', gap: '.75rem', background: selectedMethod === m.id ? 'rgba(37,99,235,0.06)' : 'rgba(255,255,255,0.03)', border: `1px solid ${selectedMethod === m.id ? '#2563eb' : 'rgba(255,255,255,0.08)'}`, borderRadius: '10px', padding: '.75rem 1rem', cursor: 'pointer', transition: 'all .2s' }}>
                    ...
                    <div style={{ width: '16px', height: '16px', borderRadius: '50%', border: `1.5px solid ${selectedMethod === m.id ? '#2563eb' : 'rgba(255,255,255,0.08)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      {selectedMethod === m.id && <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#2563eb' }} />}
                    </div>
                  </div>
                ))}
              </div>
              <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px', padding: '1rem', fontSize: '12px', color: '#8892a4', lineHeight: '1.8' }}>
                <strong style={{ color: '#e8eaf0', display: 'block', marginBottom: '.4rem' }}>How to pay:</strong>
                {[`1. Send ${price} to the number/account above`,`2. Use reference: DP-${Date.now().toString().slice(-6)}`,'3. Take a screenshot of your receipt','4. Enter your email and reference number below','5. Our team will confirm within 24 hours'].map((s, i) => <div key={i}>{s}</div>)}
              </div>
              <div style={{ marginTop: '1rem' }}>
                <label style={{ fontSize: '11px', fontWeight: 500, color: '#8892a4', display: 'block', marginBottom: '.35rem', letterSpacing: '.3px' }}>YOUR EMAIL ADDRESS</label>
                <input type="email" placeholder="so we can confirm your payment" value={email} onChange={e => setEmail(e.target.value)} style={inp} />
              </div>
              <div style={{ marginTop: '.75rem' }}>
                <label style={{ fontSize: '11px', fontWeight: 500, color: '#8892a4', display: 'block', marginBottom: '.35rem', letterSpacing: '.3px' }}>GCASH / BANK REFERENCE NUMBER</label>
                <input placeholder="e.g. 123456789012" value={ref} onChange={e => setRef(e.target.value)} style={inp} />
                <div style={{ fontSize: '11px', color: '#8892a4', marginTop: '4px' }}>Found in your GCash or banking app after payment</div>
              </div>
            </div>
            ...
            <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '14px', padding: '1.5rem' }}>
              ...
              {[['Plan',`${selectedPlan} Plan`],['Billing', annual ? 'Annual' : 'Monthly'],['Amount',price]].map(([l,v]) => (
                <div key={l} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', padding: '6px 0' }}>
                  <span style={{ color: '#8892a4' }}>{l}</span><span style={{ fontWeight: 500 }}>{v}</span>
                </div>
              ))}
              {annual && (
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', padding: '6px 0' }}>
                  <span style={{ color: '#8892a4' }}>Discount</span><span style={{ color: '#4ade80', fontWeight: 500 }}>-20%</span>
                </div>
              )}
              <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid rgba(255,255,255,0.08)', marginTop: '.5rem', paddingTop: '.75rem', fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: '16px' }}>
                <span>Total</span><span>{price}</span>
              </div>
              <div style={{ background: 'rgba(37,99,235,0.06)', border: '1px solid rgba(37,99,235,0.15)', borderRadius: '8px', padding: '.75rem 1rem', fontSize: '12px', color: '#93c5fd', lineHeight: '1.7', margin: '1rem 0' }}>
                <strong style={{ color: '#3b82f6', display: 'block', marginBottom: '.2rem' }}>📧 After payment submission:</strong>
                Our team manually verifies each payment. You'll receive a confirmation email within 24 hours, then your account will be upgraded.
              </div>
              <button onClick={handleSubmit} disabled={!email || !ref || loading} style={{ width: '100%', background: '#2563eb', color: '#fff', border: 'none', padding: '12px', borderRadius: '8px', fontSize: '14px', fontWeight: 600, cursor: (!email || !ref || loading) ? 'not-allowed' : 'pointer', opacity: (!email || !ref) ? .5 : 1, fontFamily: "'DM Sans',sans-serif", transition: 'background .2s' }}>
                {loading ? 'Submitting...' : `Submit Payment — ${price}`}
              </button>
              <div style={{ textAlign: 'center', fontSize: '11px', color: '#8892a4', marginTop: '.75rem' }}>🔒 Your payment details are kept secure</div>
            </div>
          </div>
        ) : (
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '2.5rem', textAlign: 'center' }}>
            ...
            <h2 style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: '1.5rem', marginBottom: '.5rem' }}>Payment Submitted!</h2>
            <p style={{ color: '#8892a4', fontSize: '14px', lineHeight: '1.7', marginBottom: '1.5rem' }}>We received your reference number. Our team will verify your payment and upgrade your account within 24 hours. Check your email for confirmation.</p>
            <Link href="/dashboard" style={{ display: 'inline-block', background: '#2563eb', color: '#fff', padding: '12px 32px', borderRadius: '8px', fontSize: '14px', fontWeight: 500, textDecoration: 'none' }}>Go to Dashboard →</Link>
          </div>
        )}
      </div>
    </main>
